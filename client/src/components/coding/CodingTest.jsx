// src/components/coding/CodingTest.jsx — server-graded MCQs (same as aptitude/dev)
    ...
    <div className="max-w-6xl mx-auto p-4 md:p-6">
        ...
        <div>
          ...
          <div className="flex gap-2 mt-1">
            <span className="text-xs font-bold px-2 py-0.5 rounded bg-gray-100 text-gray-600">
              Q{currentIndex + 1} of {displayQuestions.length}
            </span>
            <span
              className={`text-xs font-bold px-2 py-0.5 rounded ${
                level.difficulty === 'Hard' ? 'bg-red-100 text-red-600' : 'bg-green-100 text-green-600'
              }`}
            >
              {level.difficulty}
            </span>
      ...
      <div className={`grid grid-cols-1 ${hasCode ? 'lg:grid-cols-2' : 'max-w-3xl mx-auto'} gap-6`}>
        ...
        <div className="flex flex-col h-full space-y-6">
          ...
          {grading && (
            <p className="text-sm text-gray-600">Checking answers with the server…</p>
          )}
              ...
              {currentIndex < displayQuestions.length - 1 ? (
                ...
                !isSubmitted && (
                  <button
                    ...
                    disabled={grading}
                    className="px-8 py-2 bg-purple-600 text-white rounded-lg hover:bg-purple-700 font-bold shadow-lg transition-all disabled:opacity-50"
                  >
