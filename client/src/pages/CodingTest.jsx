...
export default function CodingTest() {
  ...
  const [testType, setTestType] = useState("dsa"); // Track which type is being used
  ...
  const fetchLevels = async () => {
    try {
      ...
      const built = available.map((lvl) => {
        ...
        return { level: lvl, accuracy: fromRec.accuracy || 0, status: fromRec.status || 'unattempted', difficulty, questionsCount: 15 };
      });
      setLevels(built);
    } catch (error) {
